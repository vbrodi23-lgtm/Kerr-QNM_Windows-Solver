...
## Fixed-Root Request Version 2

`windows-solver.fixed-root-survey-batch/2` has an explicit plan and indexed
nested sample descriptors. It does not restore `root_correction_tolerance`.
...
For the observed route the normative transition is:

`BF40 fixed-root INSUFFICIENT_ASYMPTOTIC_PRECISION`
to `PROMOTION_PENDING / RESPONSE / BF80`.
...
2. durably commit `windows-solver.promoted-exterior-control-return/2`;
3. classify with a pure registry lookup;
4. durably commit `windows-solver.promoted-exterior-control-decision/1`;
...
- ordinal 1 creates a fresh `/2` fixed-root request with no `/1` continuation
