...
- **Verification:** 216 local tests passed, including forged checkpoint centre/channel, stale projective row, resealed package, cross-package cache, and malformed input regressions; GitHub Actions run `31247921080` passed on Ubuntu and Windows before the final two review bindings were added; the final PR head must repeat every hosted gate before review readiness; board, release manifest, compile, and diff gates passed.
