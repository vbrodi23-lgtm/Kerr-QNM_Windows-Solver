...
- Traverse the unchanged campaign through mechanism-local, fail-fast mode, and ascending-coordinate chains; use only same-chain accepted response centres as execution-only PRIMARY predictors while preserving checkpoint/cache compatibility and independent SEED-PATH evidence.
